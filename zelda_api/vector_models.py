...
class IntelligenceMemo(FoundryStandardMixin, models.Model):
    ...
    # Evidence level, not a verdict. This field used to hold a four-value
    # buy-or-decline verdict generated by Claude and shown to paying investors
    # on the same page as a disclaimer saying the report is not investment
    # advice. Interlink Foundry reports what the evidence shows and leaves the
    # decision to the reader, so the field itself was replaced — renaming the
    # labels alone would have left the verdict underneath. Migration 0023 does
    # the rename and resets existing rows to NOT_CLASSIFIED, because an old
    # verdict cannot be honestly translated into an evidence rating.
    ...
    def readiness_score(self):
        """
        information_readiness is free text (Claude is prompted to format it
        ...
        """
        import re
        match = re.search(r'Score:\s*(\d{1,3})', self.information_readiness or '')
        if not match:
            return None
