...
class DealershipShowroom(models.Model):
    ...
    franchise_affiliations = models.JSONField(
        default=list, 
        help_text="Array of brands sold, e.g., Tesla, BMW, Ford."
    )
...
class TestDriveBooking(models.Model):
    ...
    # --- PINNACLE UPGRADES: Consolidated ---
    
    # Idempotency Key: Prevents duplicate API requests for the same booking
    idempotency_key = models.UUIDField(
        unique=True, 
        null=True, 
        blank=True, 
        help_text="Unique key generated by the frontend to prevent double-booking."
    )
    
    # TTL (Time-To-Live): Automated inventory release
    reservation_expires_at = models.DateTimeField(
        null=True, 
        blank=True, 
        help_text="Timestamp after which the car status reverts to 'available' if not completed."
    )
